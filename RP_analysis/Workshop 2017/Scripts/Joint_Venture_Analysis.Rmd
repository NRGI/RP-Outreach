...
jvProjects <- read_csv("Workshop Data/JV Data/jv.csv") %>% 
...
plotList <- vector("list", length = nrow(jvNames))
...
```{r}
countryList <- sort(unique(jvNames$projectCountry))

for(i in 1:length(countryList)){
    country <- countryList[i]
    projectList <- jvNames %>% 
        filter(projectCountry == country) %>% 
        select(projectName) %>% 
        unlist(use.names = F)
    
    print(country)
    
    for(j in 1:length(projectList)){
        charData <- jvProjects %>% 
            filter(projectName == projectList[j])
        ...
        print(ggplot(chartData, aes(y = value, x = reportingCompany, fill = factor(paymentType))) + 
            ...
            facet_wrap(~valueType, scales = "free_x") +
            theme_tufte() +
            labs(title = titleName, x = "Reporting Company") +
            scale_fill_viridis(discrete = T) +
            theme(
                legend.position = "top",
                legend.title = element_blank(),
